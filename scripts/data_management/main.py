    ...
    for YEAR in YEARS:
        for SCHOOL_CODE in SCHOOL_CODES:
            ...
            for suburl in ls_suburl_of_school:
                ...
                # Create the csv file
                df = pd.DataFrame(columns=['URL', 'YEAR'])
                df.to_csv(filename)
                # Start the crawling process and save results
                crawl_searcher = crawl_class(logger=logger, csvfilename=filename)
                finish_or_not = crawl_searcher.fetch_by_chatgpt(base_url, start_url, local_kw, url_filter, semantic_mining=False)
